...
############################################################################
#This rmarkdown file collected script from different authors.
#I really appreciated that they agree to make it to public.
############################################################################
...
# we retrived the immune ds from gse and nature ds
```{r}
...
# kirc
kirc_ds = readRDS("~/r_workspace/db/gse/RDS/kirc_nature_immune_os.rds")
...
kirc_ds$clin %>% 
  ...
  fun_surv_parsing(c("PFS","PFS_CNSR",1)) %>% 
  fun_surv_cutoff(feature = "ampk_score",pct = seq(0.25,0.75,0.01)) %>% 
  fun_surv_km(palette = "lancet",graph_title = "kirc_pfs") %>% 
  fun_km_re()

kirc_ds$clin %>% 
  mutate(ampk_score = kirc_clin$AMPK_score[match(rownames(.),kirc_clin$Row.names)]) %>% #colnames()
  fun_surv_parsing(c("OS","OS_CNSR",1)) %>% 
  fun_surv_cutoff(feature = "ampk_score",pct = seq(0.25,0.75,0.01)) %>% 
  fun_surv_km(palette = "lancet",graph_title = "kirc_os") %>% 
  fun_km_re()

kirc_ds$clin%>% 
  mutate(ampk_score = kirc_clin$AMPK_score[match(rownames(.),kirc_clin$Row.names)]) %>% #colnames()
  ggplot(aes(x = Benefit, y = ampk_score))+
  geom_boxplot()

kirc_ds$clin%>% 
  mutate(response = ifelse(ORR %in% c("CR","CRPR","PR"),"CR/PR",
                           ifelse(ORR %in% c("PD","SD"),"PD/SD",NA))) %>% 
  filter(!is.na(response)) %>% 
  mutate(ampk_score = kirc_clin$AMPK_score[match(rownames(.),kirc_clin$Row.names)]) %>% #colnames()
  ggplot(aes(x = response, y = ampk_score))+
  geom_boxplot()+
  stat_compare_means()
